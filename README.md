...
學生做完整份測驗，成績就會自動送到試算表，不用按任何按鈕：

| 分頁 | 內容 |
| --- | --- |
| `scores` | 總表，每次測驗一列：時間、班級、座號、姓名、單元、階段、分數、總分、百分比、基礎、進階、精熟（各段得分，例如 `9/10`）、錯題、作答秒數、attemptId |
| `班級 201` 等 | 依班級自動建立的分頁，欄位和 `scores` 相同，方便各班分開看 |
| `details` | 每題一列：段落／題型、題號、題目、正確答案、學生答案、對錯（✓／✗）、得分、提示次數 |
