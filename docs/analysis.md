...
| 3 精熟 | 看中文拼出英文；例句填空拼寫（沒有選項，提示最多 2 次，每次扣 0.25 分） | 能正確拼寫出來 |
...
- 學生：不用登入，第一次進站時填班級、座號、姓名（只存在自己的裝置）。每完成一份測驗，成績就會自動送到試算表：
  - `scores` 總表，外加依班級自動建立的「班級 201」等分頁
  - `details` 分頁記錄每一題的題目、正確答案、學生答案與對錯
  - 沒有網路時先暫存在裝置上，之後自動補送；伺服器用 `attemptId` 去除重複
  - 單字片語答錯時會立即顯示解析卡（正確答案、中文、例句，以及選錯的字的意思或拼錯的字母位置）
